...
// register Restaurant_and_Cafe_Popular_Post widget
function restaurant_and_cafe_register_popular_post_widget() {
    register_widget( 'Restaurant_and_Cafe_Popular_Post' );
}
add_action( 'widgets_init', 'restaurant_and_cafe_register_popular_post_widget' );
 
 /**
 * Adds Restaurant_and_Cafe_Popular_Post widget.
 */
class Restaurant_and_Cafe_Popular_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'restaurant_and_cafe_popular_post', // Base ID
			esc_html__( 'RARA: Popular Post', 'restaurant-and-cafe' ), // Name
			array( 'description' => esc_html__( 'A Popular Post Widget', 'restaurant-and-cafe' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
        ...
        $title      = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'Popular Posts', 'restaurant-and-cafe' );
                    ...
                    <li>
                        <?php if( has_post_thumbnail() && $show_thumb ){ ?>
                            <a href="<?php the_permalink();?>" class="post-thumbnail">
                                <?php the_post_thumbnail( 'restaurant-and-cafe-recent-post', array( 'itemprop' => 'image' ) );?>
                            </a>
                        <?php }?>
                                ...
                                <div class="entry-meta">
                                    <span class="posted-on">
                                        <a href="<?php the_permalink(); ?>">
                                            <time><?php printf( esc_html__( '%1$s', 'restaurant-and-cafe' ), get_the_date() ); ?></time>
                                        </a>                                    
                                    </span>
                                </div>
                            <?php }?>
	...
	public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'num_post' ) ); ?>"><?php esc_html_e( 'Number of Posts', 'restaurant-and-cafe' ); ?></label> 
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_postdate' ) ); ?>"><?php esc_html_e( 'Show Post Date', 'restaurant-and-cafe' ); ?></label>
		</p>
		<?php 
	}
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['title']          = ! empty( $new_instance['title'] ) ? sanitize_term_field( $new_instance['title'] ) : esc_html__( 'Popular Posts', 'restaurant-and-cafe' );
...
} // class Restaurant_and_Cafe_Popular_Post 