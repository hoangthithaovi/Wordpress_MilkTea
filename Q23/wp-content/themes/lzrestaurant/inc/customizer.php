...
function lzrestaurant_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting('lzrestaurant_theme_options',array(
        'default' => __('Right Sidebar','lzrestaurant'),
        'sanitize_callback' => 'lzrestaurant_sanitize_choices'	        
	));

	$wp_customize->add_control('lzrestaurant_theme_options',
	    array(
	        'type' => 'radio',
	...
	$wp_customize->add_section('lzrestaurant_topbar_header',array(
		...
		'description'	=> __('Add Socail Link here','lzrestaurant'),
		'priority'	=> null,
		'panel' => 'lzrestaurant_panel_id',
	));
	...
	$wp_customize->add_setting('lzrestaurant_google_plus',array(
		'default'	=> '',
		'sanitize_callback'	=> 'esc_url_raw'
	));
	
	$wp_customize->add_control('lzrestaurant_google_plus',array(
		'label'	=> __('Add Google Plus link','lzrestaurant'),
		'section'	=> 'lzrestaurant_topbar_header',
		'setting'	=> 'lzrestaurant_google_plus',
		'type'		=> 'url'
	));
	...
	for ( $count = 0; $count <= 0; $count++ ) {
		...
		$wp_customize->add_setting( 'lzrestaurant_product_page' . $count, array(
			'default'           => '',
			'sanitize_callback' => 'absint'
		));
		$wp_customize->add_control( 'lzrestaurant_product_page' . $count, array(
...
final class lzrestaurant_customize {
		...
		$manager->add_section(
			new lzrestaurant_customize_Section_Pro(
				...
				array(
					...
					'pro_url'  => esc_url('https://www.luzuk.com/themes/premium-restaurant-wordpress-theme/')
				)
			)
		);
	}
	...
	public function enqueue_control_scripts() {
		...
		wp_enqueue_script( 'lzrestaurant-customize-controls', trailingslashit( get_template_directory_uri() ) . '/assets/js/customize-controls.js', array( 'customize-controls' ) );

		wp_enqueue_style( 'lzrestaurant-customize-controls', trailingslashit( get_template_directory_uri() ) . '/assets/css/customize-controls.css' );
	}
}
